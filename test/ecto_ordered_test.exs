...
defmodule EctoOrderedTest.Scoped do
  ...
  def ranked_ids(model, [scope, scope2]) do
    new_scope = [scope: scope , scope2: scope2 ]

    (from m in model, where: ^new_scope, select: m.id, order_by: m.scoped_rank) |> Repo.all
  end
  ...
  test "multi scoped: moving between scopes" do
    ...
    scope1_model2 |> Model.changeset_multiscope(%{scoped_position: 4, scope: 2}) |> Repo.update
