...
defmodule EctoOrderedTest.Scoped do
  ...
  def ranked_ids(model, scope) do
    case scope do
      nil -> (from m in model, where: is_nil(m.scope), select: m.id, order_by: m.scoped_rank) |> Repo.all
      _   -> (from m in model, where: m.scope == ^scope, select: m.id, order_by: m.scoped_rank) |> Repo.all
    end
  end
  ...
  test "multi scoped: inserting item with no position" do
    for s <- 1..10, i <- 1..10 do
      model = Model.changeset_multiscope(%Model{scope: s, scope2: s, title: "no position, going to be ##{i}"}, %{})
      |> Repo.insert!
      assert model.scoped_rank != nil
    end
    for s <- 1..10 do
  ...
  test "multi scoped: inserting item with a correct appending position" do
    Model.changeset_multiscope(%Model{scope: 10, scope2: 100, title: "item with no position, going to be #1"}, %{})
    |> Repo.insert
    Model.changeset_multiscope(%Model{scope: 11, scope2: 100, title: "item #2"}, %{}) |> Repo.insert

    model = Model.changeset_multiscope(%Model{scope: 10, scope2: 100, title: "item #2", scoped_position: 2}, %{})
    |> Repo.insert!
  ...
  test "multi scoped: inserting item with an inserting position" do
    model1 = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "no position, going to be #1"}, %{})
    |> Repo.insert!
    model2 = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "no position, going to be #2"}, %{})
    |> Repo.insert!
    model3 = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "no position, going to be #3"}, %{})
    |> Repo.insert!

    model = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "item #2", scoped_position: 1}, %{})
    |> Repo.insert!
  ...
  test "multi scoped: inserting item with an inserting position at #1" do
    ...
    model3 = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "no position, going to be #3"}, %{})
    |> Repo.insert!
    model  = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "item #1", scoped_position: 0}, %{})
    |> Repo.insert!
  ...
  test "multi scoped: updating item with the same position" do
    model = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "no position"}, %{}) |> Repo.insert!

    model1 = Model.changeset_multiscope(model, %{title: "item with a position", scope: 1, scope2: 100})
    |> Repo.update!
    assert model.scoped_rank == model1.scoped_rank
  end
  ...
  test "multi scoped: replacing an item below" do
    model1 = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "item #1"}, %{}) |> Repo.insert!
    model2 = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "item #2"}, %{}) |> Repo.insert!
    model3 = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "item #3"}, %{}) |> Repo.insert!
    model4 = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "item #4"}, %{}) |> Repo.insert!
    model5 = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "item #5"}, %{}) |> Repo.insert!
  ...
  test "multi scoped: moving between scopes" do
    scope1_model1 = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "item #1"}, %{}) |> Repo.insert!
    scope1_model2 = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "item #2"}, %{}) |> Repo.insert!
    scope1_model3 = Model.changeset_multiscope(%Model{scope: 1, scope2: 100, title: "item #3"}, %{}) |> Repo.insert!

    scope2_model1 = Model.changeset_multiscope(%Model{scope: 2, scope2: 100, title: "item #1"}, %{}) |> Repo.insert!
    scope2_model2 = Model.changeset_multiscope(%Model{scope: 2, scope2: 100, title: "item #2"}, %{}) |> Repo.insert!
    scope2_model3 = Model.changeset_multiscope(%Model{scope: 2, scope2: 100, title: "item #3"}, %{}) |> Repo.insert!

    scope1_model2 |> Model.changeset_multiscope(%{scoped_position: 4, scope: 2, scope2: 100}) |> Repo.update

    assert Repo.get(Model, scope1_model1.id).scope == 1
    assert Repo.get(Model, scope1_model3.id).scope == 1
    assert ranked_ids(Model, [1, 100]) == [scope1_model1.id, scope1_model3.id]

    assert ranked_ids(Model, [2, 100]) == [scope2_model1.id, scope2_model2.id, scope2_model3.id, scope1_model2.id]
  end

  test "multi scoped: moving between scopes with nil values" do
    scope1_model1 = Model.changeset_multiscope(%Model{scope: 1, scope2: nil, title: "item #1"}, %{}) |> Repo.insert!
    scope1_model2 = Model.changeset_multiscope(%Model{scope: 1, scope2: nil, title: "item #2"}, %{}) |> Repo.insert!
    scope1_model3 = Model.changeset_multiscope(%Model{scope: 1, scope2: nil, title: "item #3"}, %{}) |> Repo.insert!

    scope2_model1 = Model.changeset_multiscope(%Model{scope: 2, scope2: nil, title: "item #1"}, %{}) |> Repo.insert!
    scope2_model2 = Model.changeset_multiscope(%Model{scope: 2, scope2: nil, title: "item #2"}, %{}) |> Repo.insert!
    scope2_model3 = Model.changeset_multiscope(%Model{scope: 2, scope2: nil, title: "item #3"}, %{}) |> Repo.insert!

    scope1_model2 |> Model.changeset_multiscope(%{scoped_position: 4, scope: 2, scope2: nil}) |> Repo.update

    assert Repo.get(Model, scope1_model1.id).scope == 1
    assert Repo.get(Model, scope1_model3.id).scope == 1
    assert ranked_ids(Model, [1, nil]) == [scope1_model1.id, scope1_model3.id]

    assert ranked_ids(Model, [2, nil]) == [scope2_model1.id, scope2_model2.id, scope2_model3.id, scope1_model2.id]
  end
  ## Deletion
  ...
  test "multi scoped: deleting an item" do
    model1 = Model.changeset(%Model{title: "item #1", scope: 1, scope2: 100}, %{}) |> Repo.insert!
    model2 = Model.changeset(%Model{title: "item #2", scope: 1, scope2: 100}, %{}) |> Repo.insert!
    model3 = Model.changeset(%Model{title: "item #3", scope: 1, scope2: 100}, %{}) |> Repo.insert!
    model4 = Model.changeset(%Model{title: "item #4", scope: 1, scope2: 100}, %{}) |> Repo.insert!
    model5 = Model.changeset(%Model{title: "item #5", scope: 1, scope2: 100}, %{}) |> Repo.insert!
